...
#define MAX_MOB_LEVEL 45       /* Максимальный уровень мобов */
...
{FILE *loaded;
 ...
 if (get_filename(GET_NAME(ch),filename,PMKILL_FILE) &&
   ...
   // for (i = 0; i<=MAX_MOB_LEVEL; i++) - временно убираю, нефиг 3.11.02!!!
   //   drop_old_mobs(ch, i);
}
...
{ int i;
  ...
  *(ch->MobKill.howmany+ch->MobKill.count++)  = incvalue;
  ...
  /* Проводим размакс  отключил пока что 3.11.02*/
 /* i = real_mobile(vnum);
  if (i >= 0) {
...
/* пересчет количества типов мобов каждого уровня в мире и вычисление
