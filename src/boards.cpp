...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {3004, 0, 0, LVL_GOD, LIB_ETC "board.mort", 0},
  {3048, LVL_IMMORT, LVL_IMMORT, LVL_GRGOD, LIB_ETC "board.immort", 0},
  {3078, 0, LVL_IMMORT, LVL_IMPL, LIB_ETC "board.freeze", 0},
  {3084, 0, 0, LVL_IMMORT, LIB_ETC "board.social", 0},
};
...
void init_boards(void)
{
  ...
  ACMD_READ = find_command("читать");
  ACMD_WRITE = find_command("писать");
  ACMD_REMOVE = find_command("убрать");
  ACMD_LOOK = find_command("смотреть");
  ACMD_EXAMINE = find_command("изучить");/*||find_command("examine");*/
...
int Board_write_message(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  { send_to_char("Вы недостаточно божественны, что бы делать запись на этой доске!\r\n", ch);
    return (1);
  }

  if (GET_LEVEL(ch) < 3)
  { send_to_char("Прежде чем писать на доске, Вы должны достичть 3 уровня.\r\n", ch);
    return (1);
  }

   if (PRF_FLAGGED(ch, PRF_NOWBOARD))
  { send_to_char("Вам запрещено использовать доски информации.\r\n", ch);
    return (1);
  }

  if (num_of_msgs[board_type] >= MAX_BOARD_MESSAGES)
  { send_to_char("Доска заполнена информацией полностью.\r\n", ch);
    return (1);
  }

  if ((NEW_MSG_INDEX(board_type).slot_num = find_slot()) == -1)
  { send_to_char("Доска временно не работает.\r\n", ch);
  ...
  if (!*arg) {
    send_to_char("Вы забыли указать тему сообщения!\r\n", ch);
    return (1);
  }
  ct = time(0);
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  send_to_char("Запишите свое сообщение. Что бы выйти из режима записи, введите @ на новой строке.\r\n\r\n", ch);
  act("$n начал$y писать сообщение.", TRUE, ch, 0, 0, TO_ROOM);
...
int Board_show_board(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  if (GET_LEVEL(ch) < READ_LVL(board_type)) {
    send_to_char("Вы пытаетесь прочесть сообщения, но не можете понять божественных слов.\r\n", ch);
    return (1);
  }
  act("$n начал$y читать информационную доску.", TRUE, ch, 0, 0, TO_ROOM);

  strcpy(buf,
	 "Это информационная доска. Инструкция: Читать/Убрать <сообщение #>, Писать <заголовок>.\r\n"
	 "Вы можете читать сообщения и записывать свои.\r\n");
  if (!num_of_msgs[board_type])
    strcat(buf, "На доске нет сообщений.\r\n");
  else {
    sprintf(buf + strlen(buf), "На доске имеется  %d сообщений.\r\n",
	    num_of_msgs[board_type]);
#if NEWEST_AT_TOP
	...
	send_to_char("Жаль, информационная доска временно не работает.\r\n", ch);
	return (1);
      }
    }
  }
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  if (strchr(number, '.'))	/* read 2.mail, look 2.sword */
    return (0);
  if (!isdigit((unsigned char)*number) || (!(msg = atoi(number))))
    return (0);
  ...
  if (!num_of_msgs[board_type]) {
    send_to_char("На доске нет сообщений.\r\n", ch);
    return (1);
  }
  if (msg < 1 || msg > num_of_msgs[board_type]) {
    send_to_char("Такое сообщение существует только в Вашем воображении.\r\n",
		 ch);
    return (1);
  }
#if NEWEST_AT_TOP
  ...
  if (MSG_SLOTNUM(board_type, ind) < 0 ||
    ...
    send_to_char("Извините, доска временно не работает.\r\n", ch);
    log("SYSERR: Board is screwed up. (Room #%d)", GET_ROOM_VNUM(IN_ROOM(ch)));
    return (1);
  }
  if (!(MSG_HEADING(board_type, ind))) {
    send_to_char("Это сообщение, кажется, недоступно.\r\n", ch);
    return (1);
  }
  if (!(msg_storage[MSG_SLOTNUM(board_type, ind)])) {
    send_to_char("Это сообщение, кажется, пустое.\r\n", ch);
    return (1);
  }
  sprintf(buffer, "Соообщение %d : %s\r\n\r\n%s\r\n", msg,
...
int Board_remove_msg(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  if (!*number || !isdigit((unsigned char)*number))
    return (0);
  if (!(msg = atoi(number)))
    return (0);
  ...
  if (msg < 1 || msg > num_of_msgs[board_type]) {
    send_to_char("Такое сообщение существует только в Вашем воображении -).\r\n", ch);/*That message exists only in your imagination*/
    return (1);
  }
#if NEWEST_AT_TOP
  ind = num_of_msgs[board_type] - msg;
...
#endif
  if (!MSG_HEADING(board_type, ind)) {
    send_to_char("Это сообщение, кажется, недосупно.\r\n", ch); /*That message appears to be screwed up*/
    return (1);
  }
  sprintf(buf, "(%s)", GET_NAME(ch));
  if (GET_LEVEL(ch) < REMOVE_LVL(board_type) &&
      !(strstr(MSG_HEADING(board_type, ind), buf))) {
    send_to_char("Вы не достаточно божественны, что бы удалять чужие сообщения.\r\n", ch);/*You are not holy enough to remove other people's messages*/
    return (1);
  }
  ...
  sprintf(buf, "$n удалил$y %d сообщение.", msg);
