...
void act_mtrigger(struct char_data *ch, char *str,
...
int  entry_mtrigger(struct char_data *ch, struct room_data *entry_room, int entry_dir);
int  entry_memory_mtrigger(struct char_data *ch, struct room_data *enter_from, int enter_dir);
int  greet_mtrigger(struct char_data *actor, struct room_data *enter_from, int enter_dir);
int  greet_memory_mtrigger(struct char_data *ch, struct room_data *enter_from, int enter_dir);
int  income_mtrigger(struct char_data *actor, struct room_data *enter_from, int enter_dir);
...
void random_wtrigger(struct room_data *room, int num, void * s, int types, void * list, void * next);
...
#define UID_CHAR  '\x1b'
//перенести в линух Триггера Оптимизация 14.04.2005г.
#define UID_OBJ    '\x1c'
#define UID_ROOM   '\x1d'
...
void proto_script_copy (struct trig_proto_list **pdst,
			struct trig_proto_list *src);
