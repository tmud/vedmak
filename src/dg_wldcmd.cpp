...
WCMD(do_wforce)
{
    ...
    if (!str_cmp(arg1, "all") || !str_cmp(arg1, "ЧУЕ"))
       {for (ch = room->people; ch; ch = next_ch)
	        {next_ch = ch->next_in_room;
...
WCMD(do_wdamage)
{
    ...
    if ((ch = get_char_by_room(room, name)))
       {if (GET_LEVEL(ch) >= LVL_IMMORT && dam > 0)
           {send_to_char("Это на Вас не произвело никакого эффекта.\r\n", ch);
	    return;
	   }
        GET_HIT(ch) -= dam;
        if (dam < 0)
           {send_to_char("Вы ощутили прилив жизненных сил.\r\n", ch);
            return;
           }
...
WCMD(do_wat)
{
   ...
   struct room_data *r2;
    ...
    if (!*location || !*arg2 || !isdigit((unsigned char)*location))
...
WCMD (do_wportal)
{
  ...
  act ("Лазурная пентаграмма возникла в воздухе.", FALSE,
