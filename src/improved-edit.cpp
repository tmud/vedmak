...
void send_editor_help(struct descriptor_data *d)
{
  if (using_improved_editor)
    SEND_TO_Q("Инструкция: /s или @ для сохранения, /h для вывода помощи.\r\n", d);
  else
    SEND_TO_Q("Инструкция: напечатайте @ и введите ввод.\r\n", d);
}
  ...
  *str = '\0';
  ...
  case 'c':
    if (*(d->str)) {
      free(*d->str);
      *(d->str) = NULL;
      SEND_TO_Q("Текущий буфер очищен.\r\n", d);
    } else
      SEND_TO_Q("Текущий буфер пустой.\r\n", d);
    break;
  case 'd':
...
void parse_action(int command, char *string, struct descriptor_data *d)
{
  ...
  case PARSE_HELP:
    sprintf(buf,
	    "Формат команд редактора: \"/<литер>\"\r\n\r\n"
	    "/a         -  выход из редактора\r\n"
	    "/c         -  очистка буфера\r\n"
	    "/d#        -  удалить строку #\r\n"
	    "/e# <text> -  изменить строку # на <текст>\r\n"
	    "/f         -  формат текста\r\n"
	    "/fi        -  indented formatting of text\r\n"
	    "/h         -  листинг доступных комманд\r\n"
	    "/i# <text> -  вставить <текст> после строкой #\r\n"
	    "/l         -  листинг буфера\r\n"
	    "/n         -  листинг буфера с номерами строк\r\n"
	    "/r 'a' 'b' -  заменить первое вхождение <a> в буфер на текст <b>\r\n"
	    "/ra 'a' 'b'-  заменить все вхождения текста <a> в буфере на текст <b>\r\n"
	    "              формат: /r[a] 'шаблон' 'на что заменить'\r\n"
	    "/s         -  сохранение текста\r\n");
    SEND_TO_Q(buf, d);
    break;
  case PARSE_FORMAT:
  ...
  case PARSE_REPLACE:
    ...
    if ((s = strtok(string, "'")) == NULL) {
      ...
      if ((replaced = replace_str(d->str, s, t, rep_all, d->max_str)) > 0) {
      	sprintf(buf, "Заменено вхождений '%s' на '%s'.\r\n", replaced, ((replaced != 1) ? "s " : " ") ); //, s, t);
	      SEND_TO_Q(buf, d);
    } else if (replaced == 0) {
  ...
  case PARSE_DELETE:
    ...
    case 0:
      SEND_TO_Q("Вы должны определить номер строки или диапазон, что бы стереть.\r\n", d);
      return;
    case 1:
      line_high = line_low;
      break;
	...
	SEND_TO_Q("Этот диапазон недопустим.\r\n", d);
	return;
      }
      break;
    }
  ...
  case PARSE_LIST_NORM:
    ...
    *buf = '\0';
    ...
    if (line_low < 1) {
      SEND_TO_Q("Номер строки должен быть больше чем 0.\r\n", d);
      return;
  ...
  case PARSE_LIST_NUM:
    ...
    *buf = '\0';
    ...
    if (line_low < 1) {
      SEND_TO_Q("Номер строки должен быть болье 0.\r\n", d);
      return;
    }
    if (line_high < line_low) {
      SEND_TO_Q("Этот диапазон недопустим.\r\n", d);
  ...
  case PARSE_INSERT:
	...
	*s = temp;
	SEND_TO_Q("Вставляемый текст превышает размеров буфера - прервано.\r\n", d);
	return;
      }
      if (*d->str && **d->str)
	strcat(buf, *d->str);
  ...
  case PARSE_EDIT:
    ...
    *buf = '\0';
    if ((s = *d->str) == NULL) {
      SEND_TO_Q("Буфер пусть, изенения не сделаны.\r\n", d);
      return;
    }
    if (line_low > 0) {
      /*
