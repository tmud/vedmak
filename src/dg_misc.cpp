		 ...
		 int type, char *cmd)
{
  ...
  if (!caster) 
     {caster = read_mobile(DG_CASTER_PROXY, VIRTUAL);
      ...
      if (type==OBJ_TRIGGER)
         {sprintf(buf,"дух %s",((struct obj_data *)go)->short_rdescription);
          caster->player.short_descr = str_dup(buf);
          sprintf(buf,"дух %s",((struct obj_data *)go)->short_rdescription);
         // GET_NAME(caster)  = str_dup(buf);
          sprintf(buf,"духа %s",((struct obj_data *)go)->short_rdescription);
          GET_RNAME(caster) = str_dup(buf);
          sprintf(buf,"духу %s",((struct obj_data *)go)->short_rdescription);
          GET_DNAME(caster) = str_dup(buf);
          sprintf(buf,"духа %s",((struct obj_data *)go)->short_rdescription);
          GET_VNAME(caster) = str_dup(buf);
          sprintf(buf,"духом %s",((struct obj_data *)go)->short_rdescription);
          GET_TNAME(caster) = str_dup(buf);
          sprintf(buf,"духе %s",((struct obj_data *)go)->short_rdescription);
          GET_PNAME(caster) = str_dup(buf);
         } 
      else 
      if (type==WLD_TRIGGER)
         {caster->player.short_descr = str_dup("Боги");
         //GET_INAME(caster)  = str_dup("Боги");
           GET_RNAME(caster)  = str_dup("Богов");
           GET_DNAME(caster)  = str_dup("Богам");
           GET_VNAME(caster)  = str_dup("Богов");
           GET_TNAME(caster)  = str_dup("Богами");
           GET_PNAME(caster)  = str_dup("Богах");
         } 
