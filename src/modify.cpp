...
void string_add(struct descriptor_data *d, char *str)
{
  ...
  if (!(*d->str)) {
    if (strlen(str) + 3 > d->max_str) {
      send_to_char("Длинная строка - Обрезаем.\r\n", d->character);
    ...
    if (strlen(str) + strlen(*d->str) + 3 > d->max_str) {
      send_to_char("Слишком длинная строка. Последняя строка пропущена.\r\n", d->character);
      if (!using_improved_editor)
        action = STRINGADD_SAVE;
    } else {
...
void playing_string_cleanup(struct descriptor_data *d, int action)
{
  if (PLR_FLAGGED(d->character, PLR_MAILING)) {
    ...
    {      
      ...
      SEND_TO_Q("Письмо отправлено!\r\n", d);
    } else
      SEND_TO_Q("Отправка почты прервана.\r\n", d);
    free(*d->str);
    free(d->str);
  }
...
ACMD(do_skillset)
{
  ...
  if (!*name) {			/* no arguments. print an informative text */
    send_to_char("Формат: skillset <имя_игрока> '<имя_скилла>' <процент>\r\n", ch);
    strcpy(help, "\\c14Вы владеете следующими заклинаниями:\\c00\r\n");
...
strcpy(help, "\r\n\\c13Вы владеете следующими умениями:\\c00\r\n");
  ...
  if (!*argument) {
    send_to_char("Введите название скилла.\r\n", ch);
    return;
  }
  if (*argument != '\'') {
    send_to_char("Умение должно быть заключено в: ''\r\n", ch);
    return;
  }
  ...
  if (skill < 0 && spell < 0) {
    send_to_char("Несуществующее умение.\r\n", ch);
    return;
  }
  ...
  if (!*buf) {
    send_to_char("На сколько Вы хотите поднять умение?\r\n", ch);
    return;
  }
  value = atoi(buf);
  if (value < 0) {
    send_to_char("Минимальный уровень умений 0 процентов.\r\n", ch);
    return;
  }
  if (value > 100) {
    send_to_char("Максимальный уровень умений 100 процентов.\r\n", ch);
    return;
  }
  if (IS_NPC(vict)) {
 SET_SKILL(vict, skill, value);
sprintf(buf, "Вы изменили умение мобу до %d процентов.\r\n", value);
send_to_char(buf, ch);
   /* send_to_char("Вы не можете присвоить скилл NPC (мобу).\r\n", ch);*/
    return;
  }
...
sprintf(buf2, "%s changed %s's %s to %d.", GET_NAME(ch), GET_NAME(vict),
  ...
  sprintf(buf2, "Вы изменили %s умение \"%s\" до %d процентов.\r\n", GET_DNAME(vict),
...
void show_string(struct descriptor_data *d, char *input)
{
   ...
   */
    ...
    send_to_char(
		  "&KДля продолжения нажмите: Enter, (к)онец, (п)овтор, (н)азад, или номер страницы.&n\r\n",
		  d->character);
    return;
  }
