...
int do_social(struct char_data *ch, char *argument)
{
  ...
  if (!*buf) 
     { if (action->char_self)
		  act(action->char_self, FALSE, ch, 0, 0, TO_CHAR);
       if (action->char_self_room)
		  act(action->char_self_room, FALSE, ch, 0, 0, TO_ROOM);
        return(TRUE);
     }
  if (!(vict = get_char_vis(ch, buf, FIND_CHAR_ROOM))) 
     { if (action->others_found)
       act(action->others_found, FALSE, ch, 0, 0, TO_CHAR);
     } 
  else 
  if (vict == ch) 
     { if (action->vict_found)
		  act(action->vict_found, FALSE, ch, 0, vict, TO_CHAR);
       if (action->not_found)
		  act(action->not_found, FALSE, ch, 0, vict, TO_ROOM);
     } 
  else 
     {if (GET_POS(vict) < action->vict_min_pos || GET_POS(vict) > action->vict_max_pos)
         act("$N сейчас не в том состоянии что бы обращать внимание на Вас.",
 	         FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);
      else 
         {if (action->char_no_arg)
		  act(action->char_no_arg, FALSE, ch, 0, vict, TO_CHAR);
          if (action->others_no_arg)
		  act(action->others_no_arg, FALSE, ch, 0, vict, TO_NOTVICT);
	      if (action->char_found)
		  act(action->char_found, FALSE, ch, 0, vict, TO_VICT);
         }
     }
  return (TRUE);   
}
  ...
  ACMD(do_insult)
{
  ...
  if (*arg) {
    if (!(victim = get_char_vis(ch, arg, FIND_CHAR_ROOM)))
      send_to_char("Не может слышать Вас!\r\n", ch);
    else {
      if (victim != ch) {
	...
	case 0:
	  if (GET_SEX(ch) == SEX_MALE) {
	    if (GET_SEX(victim) == SEX_MALE)
	      act("$n сказал$y Вам, что Вы сражаетесь как женщина!", FALSE, ch, 0, victim, TO_VICT);
	    else
	      act("$n сказал$y Вам - женщины не могут сражаться.", FALSE, ch, 0, victim, TO_VICT);
	  } else {		/* Ch == Woman */
	    if (GET_SEX(victim) == SEX_MALE)
	      act("$n обвинил$y Вас в тупости...",
		  FALSE, ch, 0, victim, TO_VICT);
	    else
	      act("$n сказал$y Вам, что тролль по сравнению с Вами красавица.",
		  FALSE, ch, 0, victim, TO_VICT);
	  }
	  break;
	case 1:
	  act("$n обозвал$y Вашу мать доской!", FALSE, ch, 0, victim, TO_VICT);
	  break;
	default:
	  act("$n сообщил$y, что Вы опущенный человек!", FALSE, ch, 0, victim, TO_VICT);
	  break;
	}			/* end switch */

	act("$n оскорбил$y $V.", TRUE, ch, 0, victim, TO_NOTVICT);
      } else {			/* ch == victim */
	send_to_char("Вас оскорбили и Вы обтекаете.\r\n", ch);
      }
    }
  } else
    send_to_char("Я уверен, что вы не хотите оскорблений *everybody*...\r\n", ch);
}
             ...
             if (scan && *scan && *scan != ';')
                {switch (what)
                 {case 0:
                  ...
                  break;
				  case 1:
                  soc_mess_list[message].char_self = str_dup_bl(scan);
                  break;
                  case 2:
                  soc_mess_list[message].char_self_room = str_dup_bl(scan);
				  break;
                  case 3:
