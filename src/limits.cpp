...
int hit_gain(struct char_data * ch)
{
 ...
 gain = graf(age(ch)->year, restore-3, restore, restore, restore-2, restore-3, restore-5, restore-7);
  ...
  if (AFF_FLAGGED(ch, AFF_POISON) && percent > 0)
     percent /= 4;
  percent  = MAX(-200, MIN(3000, percent));//отметка
  gain     = gain * percent / 100;
  if (!IS_NPC(ch))
     ...
     {next_char = i->next;
      ...
      if (PLR_FLAGGED(i, PLR_HELLED) &&
         ...
         {restore = PLR_TOG_CHK(i, PLR_HELLED);
          send_to_char("&GВас выпустили из АДА.&n\r\n",i);
          ...
          act("$n появил$u в центре комнаты, с радостным выражением лица!",FALSE,i,0,0,TO_ROOM);
         }
      if (PLR_FLAGGED(i, PLR_NAMED) &&
         ...
         {restore = PLR_TOG_CHK(i, PLR_NAMED);
          send_to_char("&GВас выпустили из КОМНАТЫ ИМЕНИ.&n\r\n",i);
          if ((restore = GET_LOADROOM(i)) == NOWHERE)
             restore = calc_loadroom(i);
          restore = real_room(restore);
          if (restore == NOWHERE)
          ...
          char_to_room(i,restore);
          look_at_room(i,restore);
          act("С поправленным именем, $v выпустили из \"комнаты имени\".",FALSE,i,0,0,TO_ROOM);
         }
		...
		{  
			...
			if (affected_by_spell(i, SPELL_ZNAK_IRGEN)) {
			    ...
			    { affect_from_char(i, SPELL_ZNAK_IRGEN);
                             send_to_char("Защита знака &Wирген&n исчезла.\r\n", i);	
			    }
			}
		    ...
		    if (affected_by_spell(i, SPELL_ZNAK_AKSY))
			    ...
			    { affect_from_char(i, SPELL_ZNAK_AKSY);
			      send_to_char("Защита знака &Wаксий&n исчезла.\r\n", i);
			    }
			...
			if (affected_by_spell(i, SPELL_ZNAK_GELIOTROP)) 
			{
			   ...
			   { affect_from_char(i, SPELL_ZNAK_GELIOTROP);
			     send_to_char("Защита знака &Wгелиотроп&n исчезла.\r\n", i);
			   }
			}
			...
			if (affected_by_spell(i, SPELL_ZNAK_KVEN))
			{
			   ...
			   { affect_from_char(i, SPELL_ZNAK_KVEN);
			     send_to_char("Защита знака &Wквен&n исчезла.\r\n", i);
			   }
			}
       ...
       // Restore PC caster mem
        ...
        {            
            ...
            if (next_spell != -1)
            {
                ...
                sprintf(buf2,"Вы вновь можете воспользоваться заклинанием '%s%s%s'!\r\n", CCRED(i,C_NRM), 
               ...
               {GET_MANA_STORED(i) = 0;
                ...
                //if (GET_RELIGION(i) == RELIGION_MONO)
                  if ((GET_CLASS(i) == CLASS_CLERIC) || (GET_CLASS(i) == CLASS_SLEDOPYT))  
				     sprintf(buf2,"Ваши занятия окончены. Вы с улыбкой убрали свой манускприпт.\r\n");
                else
                  if((GET_CLASS(i) == CLASS_MAGIC_USER) || (GET_CLASS(i) == CLASS_TAMPLIER) || (GET_CLASS(i) == CLASS_ASSASINE))
				    sprintf(buf2,"Ваши занятия окончены. Вы с улыбкой захлопнули свою книгу заклинаний.\r\n");
                else
				  if((GET_CLASS(i) == CLASS_DRUID) || (GET_CLASS(i) == CLASS_MONAH))
					sprintf(buf2,"Ваши занятия окончены. Вы с улыбкой спрятали свои таблички.\r\n");
                 ...
                 // if (GET_RELIGION(i) == RELIGION_MONO)
                 //   sprintf(buf2,"Окончив занятия, $n с улыбкой захлопнул$y книгу заклинаний.");
                 //else
                 //  sprintf(buf2,"Окончив занятия, $n с улыбкой убрал$y книгу заклинаний.");

                 if ((GET_CLASS(i) == CLASS_CLERIC) || (GET_CLASS(i) == CLASS_SLEDOPYT))  
				     sprintf(buf2,"Закончив занятия, $n с улыбкой убрал$y свой манускрипт.");
                else
                  if((GET_CLASS(i) == CLASS_MAGIC_USER) || (GET_CLASS(i) == CLASS_TAMPLIER) || (GET_CLASS(i) == CLASS_ASSASINE))
				    sprintf(buf2,"Закончив занятия, $n с улыбкой захлопнул$y свою книгу заклинаний.");
                else
				  if((GET_CLASS(i) == CLASS_DRUID) || (GET_CLASS(i) == CLASS_MONAH))
					sprintf(buf2,"Закончив занятия, $n с улыбкой спрятал$y свои таблички.");
...
void gain_exp(struct char_data * ch, int gain)
{
  ...
  if (gain > 0)
  {
      ...
      // опыт не идет если достигли уровня реморта - отключено
      //GET_EXP(ch) = MIN(GET_EXP(ch), level_exp(GET_CLASS(ch),LVL_REMORT + GET_REMORT(ch)) - 1);
            
      // нету левел апа если получено право на реморт, но копим опыт
      if (!GET_GOD_FLAG(ch, GF_REMORT))      
      {
                 ...
                 GET_EXP(ch) >= level_exp(GET_CLASS(ch), GET_LEVEL(ch) + 1))
          {
              GET_LEVEL(ch) += 1;
              num_levels++;
              sprintf(buf,"&GПоздравляем, вы получили УРОВЕНЬ!&n\r\n");
              send_to_char(buf,ch);
    	    
              if (GET_LEVEL(ch)==16)
	             send_to_char("&WВы получили право носить титул!&n\r\n", ch);
		      if (GET_LEVEL(ch)==24)
	             send_to_char("&WВы получили право носить предтитул!&n\r\n", ch);           
              advance_level(ch);
              is_altered = TRUE;
          }
          ...
          if (GET_EXP(ch) >= level_exp(GET_CLASS(ch), LVL_REMORT + GET_REMORT(ch)))
          {
              ...
              {
                  sprintf(buf,"%s&MВы получили право на перевоплощение!&n%s\r\n", CCGRN(ch,C_NRM), CCNRM(ch,C_NRM));
                  send_to_char(buf,ch);
  ...
  else if (gain < 0)
  {
             ...
             (GET_EXP(ch)+ GET_EXP(ch)/3) < level_exp(GET_CLASS(ch), GET_LEVEL(ch)))
      {
          GET_LEVEL(ch) -= 1;
          num_levels++;
          sprintf(buf,"%sВы потеряли уровень!%s\r\n", CCRED(ch, C_NRM), CCNRM(ch, C_NRM));
          send_to_char(buf,ch);
          decrease_level(ch);
          is_altered = TRUE;
      }
      ...
      {
          sprintf(buf,"%sВы утратили право на перевоплощение!%s\r\n", CCGRN(ch,C_NRM),CCNRM(ch,C_NRM));
          send_to_char(buf,ch);
...
void gain_exp_regardless(struct char_data * ch, int gain)
{
  ...
  if (!IS_NPC(ch)) {
    ...
    if (is_altered) {
      sprintf(buf, "%s поднялся на %d уровен%s и сейчас имеет уровень - %d.",
		GET_NAME(ch), num_levels, num_levels == 1 ? "ь" : "ей",
		GET_LEVEL(ch));
      mudlog(buf, BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE);
      if (num_levels == 1)
        send_to_char("&GВы поднялись на уровень!&n\r\n", ch);
      else {
	sprintf(buf, "Вы поднялись на %d %s!\r\n", num_levels, desc_count(num_levels, WHAT_LEVEL));
	send_to_char(buf, ch);
			}
	  /*    set_title(ch, NULL);*/
      check_autowiz(ch);
	...
	// разобраться с бонусами!!!
/*	if(!ch->player.unbonus && GET_LEVEL(ch) == 16)
		   { ch->player.unbonus =1;
			 ch->player.bonus++;
			 sprintf(buf, "%sВам прибавилась тренировочная единица!%s\r\n",CCGRN(ch,CMP), CCNRM(ch,CMP));
			 send_to_char(buf, ch);
		   }
...
void check_idling(struct char_data * ch)
{
  if (!RENTABLE(ch)) {
	...
	stop_fighting(ch, TRUE);
      ...
      act("$n медленно растворил$u в воздухе.", TRUE, ch, 0, 0, TO_ROOM);
      send_to_char("Вы медленно растворились в воздухе.\r\n", ch);
	...
	Crash_idlesave(ch);
      ...
      sprintf(buf, "%s персонаж принудительно заренчен и удален (освобожден).", GET_NAME(ch));
 ...
 for (i = character_list; i; i = next_char) {
	...
	if (AFF_FLAGGED(i, AFF_SLEEP) && GET_POS(i) > POS_SLEEPING)
		{ GET_POS(i) = POS_SLEEPING;
		  send_to_char("Война войной, а сон по расписанию!\r\n", i);
		  act("$n сладострастно потянул$u и громко захропел$y, повернувшись на другой бок.", TRUE, i, 0, 0, TO_ROOM);
		}
	  ...
	  {   GET_TIME_KILL(i) --;
			...
			if (GET_TIME_KILL(i) <=0 )
				{ GET_TIME_KILL(i) = 0;
				  ...
				  send_to_char("Флаг УБИЙЦЫ пропал.\r\n",i);
				}
      }
	}
 ...
 // Restore mobs
          if (IS_NPC(i) && !FIGHTING(i))
		   ...
		   // Forget PC's
                for (mem = MEMORY(i),pmem=NULL; mem; mem=nmem)
                  {nmem = mem->next;
                   if (mem->time <= 0 && FIGHTING(i))
                     { pmem = mem;//добавил - может не будет глючить. протестить.
					   continue;
					 }
			  ...
			  // Remember some spells тут надо проверять метку, моб кастер или нет. что бы не грузить движок
...
// rooms 
      ...
      {if (world[count].fires)
          {switch (get_room_sky(count))
           ...
           if (world[count].fires <= 0)
              {act("Костер совсем потух.",FALSE,world[count].people,0,0,TO_ROOM);
               act("Костер совсем потух.",FALSE,world[count].people,0,0,TO_CHAR);
               world[count].fires = 0;
              }
          }
	   ...
	   { world[count].portal_time--;
            ...
            {act("Магический портал задрожал и растворился в пространстве.",FALSE,world[count].people,0,0,TO_ROOM);
             act("Магический портал задрожал и растворился в пространстве.",FALSE,world[count].people,0,0,TO_CHAR);
            }  
		}
       if (world[count].ices)
          if (!--world[count].ices)
  ...
  {   next_thing = j->next;	// следующий предмет из списка
    // If this is a corpse 
	...
	{ // timer count down
	  ...
	  { for (jj = j->contains; jj; jj = next_thing2)
		{ next_thing2 = jj->next_content;	// Next in inventory 
	  	...
	  	if (j->carried_by)
			{ act("$3 распал$U в Ваших руках.", FALSE, j->carried_by, j, 0, TO_CHAR);
			  obj_from_char(j);
			}
		else
	  	if ((j->in_room != NOWHERE))
	   	{ if (world[j->in_room].people)
			{ act("Налетевший рой мух ничего не оставил от $1.",
				TRUE, world[j->in_room].people, j, 0, TO_ROOM);
			 act("Налетевший рой мух ничего не оставил от $1.",
				TRUE, world[j->in_room].people, j, 0, TO_CHAR);
			}
	    obj_from_room(j);//протестить но скорее всего эту ситуацию прослеживал, вроде был креш
	  	}					//двойное удаление получалось. проверить, бросив труп с предметом и исследовать что будет.
	  	else
		  if (j->in_obj)
		   obj_from_obj(j);
	           extract_obj(j);
 ...
 /* If the timer is set, count it down and at 0, try the trigger */
    /* note to .rej hand-patchers: make this last in your point-update() */
          ...
          {if (SCRIPT_CHECK(j, OTRIG_TIMER))
           ...
           else // РАссыпающисе предметы от таймера на земле, за исключением если рума используется как склад. 
               ...
               if (j->worn_by)
                  {switch (j->worn_on)
                   {case WEAR_LIGHT :
                    ...
                    case WEAR_HOLD :
                    case WEAR_BOTHS :
                         act("$o рассыпал$U в Ваших руках, превратившись в пыль.",FALSE,j->worn_by,j,0,TO_CHAR);
                         break;
                    default :
                         act("$o рассыпал$U на Вашем теле...",FALSE,j->worn_by,j,0,TO_CHAR);
                         break;
                   }
		   unequip_char(j->worn_by,j->worn_on);
                  }
               else   if (j->carried_by)
			{ act("$o рассыпал$U в Ваших руках, превратившись в пыль.",FALSE,j->carried_by,j,0,TO_CHAR);
			  obj_from_char(j);
			}
		  else
		     if (j->in_room != NOWHERE)
	              {if (world[j->in_room].people)
			{act("$o превратил$U в пыль, которую развеял ветер.",
 ...
 // Тонущие, падающие, и сыпящиеся обьекты.
  for (j = object_list; j; j = next_thing)
  { next_thing = j->next;	// следующий объект по списку
    if (j->contains)
	{
     cont = TRUE;
    } else
...
void set_afk(struct char_data * ch, char *afk_message, int result)
{
  ...
  if (strlen(buf1) > MAX_POOF_LENGTH) {
    send_to_char("Сообщение АФК слишком длинное, сделайте его короче.\r\n", ch);
    sprintf(buf1, "%s", AFK_DEFAULT);
  }
  ...
  if (result) {
     sprintf(buf, "Вы ушли в режим AFK:\\c02 %s\\c00", CAP(GET_AFK(ch)));
		act(buf, FALSE, ch, 0, 0, TO_CHAR);
  } else {
		act("$n вернул$u из AFK!", FALSE, ch, 0, 0, TO_ROOM);
		act("Вы вернулись из AFK!", FALSE, ch, 0, 0, TO_CHAR);
		}
}
	...
	for (obj = object_list; obj; obj = obj_next)
		{ obj_next = obj->next;
	      ...
	      if ((int)GET_OBJ_VNUM(obj)/100 == zone_table[zone].number && OBJ_FLAGGED(obj, ITEM_ZONERESET))
		  	{  
			  if (obj->worn_by)
				{ act("$o рассыпал$U на глазах у Всех.", FALSE,
				  obj->worn_by, obj, 0, TO_ROOM);
				  act("$o рассыпал$U в Ваших руках.", FALSE,
				...
				{ if (world[obj->in_room].people)
					{ act ("$o рассыпал$U, обдав Вас пылью...",
