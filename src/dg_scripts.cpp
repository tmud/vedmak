...
//перенести в линух ОЛЦ 12.04.2005г.
...
obj_data *get_obj_by_char(struct char_data *ch, char *name)
{
	...
	if ((*name == UID_ROOM) || (*name == UID_CHAR))//перенести в линух Триггер 14.04.2005г.
    return NULL;
...
void script_stat (char_data *ch, struct script_data *sc)
{
  ...
  sprintf(buf, "Глобальные Переменные: %s\r\n", sc->global_vars ? "" : "Нет");
  send_to_char(buf, ch);
  ...
  for (tv = sc->global_vars; tv; tv = tv->next)
      {sprintf(namebuf,"%s:%ld", tv->name, tv->context);
       if (*(tv->value) == UID_CHAR || *(tv->value) == UID_ROOM//перенести в линух Триггер 14.04.2005г.
	  || *(tv->value) == UID_OBJ)
  ...
  for (t = TRIGGERS(sc); t; t = t->next)
      {sprintf(buf, "\r\n  Триггер: %s%s%s, VNum: [%s%5d%s], RNum: [%5d]\r\n",
       ...
       sprintf(buf, "  Тип Триггера: %s, Числовой Аргумент: %d, Список Аргументов: %s\r\n",
       ...
       if (GET_TRIG_WAIT(t))
          {sprintf(buf, "    Ожидание: %d, Следующая команда: %s\r\n",
		  GET_TRIG_WAIT(t)->time_remaining, t->curr_state ? t->curr_state->cmd : "Нет");
           send_to_char(buf, ch);

           sprintf(buf, "  Переменные: %s\r\n", GET_TRIG_VARS(t) ? "" : "Нет");
           send_to_char(buf, ch);
...
void do_sstat_room(struct char_data * ch)
{
  ...
  send_to_char("\r\nИнформация о скрипте (триггере):\r\n", ch);
  if (!SCRIPT(rm))
     {send_to_char("  Отсутствует.\r\n", ch);
      return;
     }
...
// разобраться .. тут так.. временно принудительно включил триггер (SCRIPT_TYPES(sc) |= MTRIG_LOAD;)...
  //должен он в зон файл загружаться...
      ...
      if (is_abbrev(arg1, "object"))
         ...
         {if ((object = get_object_in_equip_vis(ch, arg1, ch->equipment, &tmp)))
          ...
          if ((object = get_obj_vis(ch, arg1)))
             ...
             send_to_char("Здесь нет ничего с подобным именем.\r\n", ch);
          trigger = arg2;
         }

      if (victim)
         {if (!IS_NPC(victim))
             send_to_char("Игроки не могут иметь триггера.\r\n", ch);
          else
          if (!SCRIPT(victim))
             send_to_char("Мобы не имеют никаких триггеров.\r\n", ch);
          else
          if (!str_cmp(arg2, "all") || !str_cmp(arg2, "все"))
...
--*/
{
   ...
   if ( !vd ) return 0;				// Не удалось найти
 ...
 */
/*int remove_var(struct trig_var_data **var_list, char *name)триггера
{
...
*/                      
 ...
 /*  if (!vd)
  ...
  if ( !subfield ) subfield = ""; // Чтобы проверок меньше было
  ...
  if (!field || !*field) {
     ...
     if (vd) {
        ...
        switch (type) {
          case MOB_TRIGGER:
               ...
               if ((c = get_char_room(name, IN_ROOM(ch)))) //проверить
	              ;
               else
               if ((o = get_obj_in_list(name,world[IN_ROOM(ch)].contents)))
                  ;
               else
               if ((c = get_char(name))) //изменить в функции
                  ;
               else
               if ((o = get_obj(name)))
                  ;
     ...
     if (c) {
        ...
        if (!str_cmp(field, "gold")) {
		 ...
		 if (!str_cmp(field, "clan"))  //идешник клана
           sprintf(str, "%d", GET_CLAN(c));
		 else
         if (!str_cmp(field, "clanrank")) //ранг кланового игрока
		...
		{	if((clan_num=find_clan_by_id(GET_CLAN(c)))<0)
         ...
         if (!str_cmp(field, "clanbank")) {// начисление клан_денег
			if((clan_num=find_clan_by_id(GET_CLAN(c)))<0)
				return;
			 if (*subfield) {
               if (*subfield == '-')
	  ...
	  { if((clan_num=find_clan_by_id(GET_CLAN(c)))<0)
         ...
         if (!str_cmp(field, "race"))//доработать с выдачей не числа а названия.
        ...
        if (!str_cmp(field, "affect")){
           ...
           if (subfield) {
              if ((flag = atoi(subfield)) || (flag = ext_search_block(subfield,affected_bits,FALSE))) {
                 ...
                 for (i = 0;i < 2;i++) {
                     if (AFF_FLAGGED(c,(flags[i] | mask[i]))) {//протестить 25.05.2004
                        strcpy(str,"1");
                        break;
                     }
                 }
	...
	//	if (c->master)
       //      sprintf(str,"%c%ld",UID_CHAR, GET_ID(c->master));
        ...
        if (!str_cmp(field, "can_help")) {
                ...
                !str_cmp(field, "all") )
           {
              ...
              if ( inroom == NOWHERE )
              {
                 script_log( "mob-построитель списка в NOWHERE" );
                 return;
              }
     ...
     if (o) {
        ...
        if (!str_cmp(field, "room"))
		{  obj_data *obj = o; //ошибка не было скобки.
           while (obj->in_obj)
                 obj = obj->in_obj;
           if (obj->carried_by)
             ...
             !str_cmp(field, "all") )
        {
           ...
           if ( inroom == NOWHERE )
           {
              script_log( "obj-построитель списка в NOWHERE" );
              return;
           }
    ...
    if (r) {
       ...
       if (!str_cmp(field, "id")) {
          sprintf(str,"%c%d", UID_ROOM, find_room_uid(r->number));//перенести в линух Триггера 14.04.2005г.
       }
       else
       if (!str_cmp(field, "people")) {
          if (r->people)
	   ...
	   {    int inroom = real_room( r->number );
		   char_data *rndm = NULL;
            if ( inroom == NOWHERE )
	   { script_log( "room-построитель списка в NOWHERE" );
              return;
           }
           for ( rndm = world[inroom].people; rndm; rndm = rndm->next_in_room )
		   {
...
int is_num(char *num)
{	//временно заменил проверку числа на проверку не числа -)
	       ...
	       trig_data *trig, int type)
{
  ...
  while (*line && is_space(*line))//пропускаем пробелы
        line++;
...
--*/
{
  ...
  /* if ( v )
   {//разобраться в не правильной обработке ниженаписанной фигни.
   ...
   p = value;
   while ( *list && is_space(*list) ) ++list;          // пропуск пробелов
   while ( *list && !is_space(*list) ) *p++ = *list++; // копирование слова
   *p = 0;
		  ...
		  struct cmdlist_element *cl)
{
  ...
  extern unsigned long dg_global_pulse;
   ...
   if (trig->attach_type == MOB_TRIGGER &&
    ...
    {
      sprintf (buf,
	       "&CИспользуется wait в DEATH триггере '%s' (VNUM=%d).&n",
	       GET_TRIG_NAME (trig), GET_TRIG_VNUM (trig));
      mudlog (buf, BRF, LVL_BUILDER, TRUE);
      sprintf (buf, "&CКод триггера после wait выполнен НЕ БУДЕТ!&n");
      mudlog (buf, BRF, LVL_BUILDER, TRUE);
    }
		         ...
		         int type, char *cmd)
{
  ...
  add_var_cntx(&GET_TRIG_VARS(trig), varname, uid, sc->context);//триггера
}
...
void process_remote(struct script_data *sc, trig_data *trig, char *cmd)
{
  ...
  vd = find_var_cntx( &GET_TRIG_VARS(trig), var, 0 );//триггера
  if ( !vd )
   vd = find_var_cntx( &(sc->global_vars), var, sc->context );
...
// "покрывает" отсутствующие контексты ПРОТЕСТИТЬ
  context = sc->context;
               ...
               if (!strn_cmp(cmd, "version", 7))
                  mudlog(DG_SCRIPT_VERSION, NRM, LVL_GOD, /*SYSLOG,*/ TRUE);
                     ...
                     if (dg_owner_purged)
                        {depth--;
                         cur_trig = prev_trig;
                         return ret_val;//0;//разобраться (не верно работает при удалении предмета на земле и снятия с него триггера)
                        }
                    }
              }
      }
...
{ int first, last, nr, found = 0;
  ...
  if (!*buf)
     {send_to_char("Формат: tlist [тлист] <начальный номер зоны> [<конечный номер>]\r\n", ch);
      return;
     }
  ...
  if ((first < 0) || (first > 999999) || (last < 0) || (last > 999999))// расширение до 999999
     {send_to_char("Значения долны лежать в диапазоне от 0 до 999999.\n\r", ch);
      return;
     }

  if (first >= last)
     {send_to_char("Второе значение должно быть больше чем первое.\n\r", ch);
      return;
     }
  ...
  if (!found)
     send_to_char("Нет триггеров в заданном диапазоне значений.\n\r", ch);
  else
...
ACMD(do_tstat)
{
  ...
  if (*str)
     {vnum = atoi(str);
      ...
      if (rnum<0)
         {send_to_char("Несуществующий номер триггера.\r\n", ch);
          return;
         }
...
void read_saved_vars(struct char_data *ch)
{
  ...
  do {if (fgetline(file, input_line)>0)
          ...
          *c = '\0';
          ...
          add_var_cntx(&(SCRIPT(ch)->global_vars), varname, p, context);//триггера
         }
     } while( !feof(file) );
