 ...
 *
  ...
  Ниже - несколько констант, которые Вы можете изменить, чтобы изменить определенные аспекты
  из пути действия CircleMUD. Так как это - .c файл, все, что Вы должны сделать
  изменять одну из констант (принимающий{предполагающий} Вас имеют в наличии ваши файлы объекта{цели}), 
  является изменением{заменой}, которое постоянные{неизменные} в этом файле и типе 'делают'. Сделайте повторно соберет
  этот файл и пересвязь; Вы не должны ждать всего этого к
  повторно соберите, поскольку Вы делаете, если Вы изменяете файл удара головой{заголовка}.
 
  Я понимаю, что было бы немного более эффективно иметь много из
  #defines усыпанный о, так, чтобы, например, кодекс{код} авточеловека выдающихся способностей не
  собранный вообще, если Вы не хотите использовать авточеловека выдающихся способностей. Однако, фактическое
  кодекс{код} для различных вариантов является весьма маленьким, как - вычислительное время
  в проверке выбора{опции} Вы выбрали во времени выполнения, таким образом я решил
  удобство наличия всех ваших вариантов в этом файле перевешивает
  efficency выполнения этого другой путь.
...
// Позволино ли игроку воровать? 
int pt_allowed = YES;

// minimum level a player must be to shout/holler/gossip/auction 
int level_can_shout = 1;

// Показывает, сколько теряется мувов за один крик
int holler_move_cost = 20;

// exp change limits 
int max_exp_gain_npc = 100000;	// Максимальное получение экспы за моба 
int max_exp_loss = 4000000;	// Потеря экспы при смерти (max) 
...
// Установка времени перезагрузки 
int reboot_time = 6;

// Установка значения условия перезагрузки
int reboot_state = 0;
...
// "okay" etc.
const char *OK = "Исполнено, Повелитель!\r\n";
const char *NOPERSON = "Здесь нет никого с таким именем.\r\n";
const char *NOEFFECT = "Ничего, кажется, не произошло.\r\n"; 
...
room_vnum mortal_start_room = 3625;

/* virtual number of room that immorts should enter at by default */
room_vnum immort_start_room = 1201;

/* virtual number of room that frozen players should enter at */
room_vnum frozen_start_room = 1202; /* для пофиризиных надо изменить*/

room_vnum helled_start_room = 1202;  /* комната для ада */
room_vnum named_start_room  = 1207;
room_vnum unreg_start_room  = 1208;
/*
 ...
 * donation rooms.
 */
room_vnum donation_room_1 = 1200;
...
"   &gЕсли Вы читаете это сообщение, возможно, Вы впервые попали в &RМир Ведьмака&g.\r\n"
"   &gНаш сервер является бесплатным и независимым, с большой историей и традициями.\r\n"
"   &gИграя Вы можете приобрести интересное хобби, получать адреналин и многое другое.\r\n"
"   &gОбщаясь с другими игроками - новых друзей, впечатления и яркие воспоминания.\r\n"
"   &gВы появитесь в Мире в славном городе Шаэрраведде, желаем удачи и приключений.\r\n"
"   &gНовичкам в MUD рекомендуем сразу же набрать &W'войти школа'&n &gдля обучения игре&g.&n\r\n\r\n";
...
{ if (IS_IMMORTAL(ch))
  ...
  if (PLR_FLAGGED(ch, PLR_HELLED))
     return (helled_start_room);
  else
  if (PLR_FLAGGED(ch, PLR_NAMED))
	  return (named_start_room);
 /* else
  if (PLR_FLAGGED(ch, PLR_REGISTERED))
	  return (unreg_start_room);*/
  else
  if (GET_LEVEL(ch) < 15) //разобраться в дальнейшем исправить
     {switch(GET_CLASS(ch))
      { case CLASS_MAGIC_USER: return 3625; break;
        case CLASS_CLERIC:     return 3625; break;
        case CLASS_THIEF:      return 3625; break;
        case CLASS_WARRIOR:    return 3625; break;
        case CLASS_VARVAR:     return 3625; break;
        case CLASS_SLEDOPYT:   return 3625; break;
	case CLASS_TAMPLIER:   return 3625; break;
        case CLASS_MONAH:      return 3625; break;
	case CLASS_ASSASINE:   return 3625; break;
	case CLASS_WEDMAK:     return 3625; break;
      }
     }
