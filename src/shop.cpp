...
void mort_show_obj_values(struct obj_data *obj, struct char_data *ch, int lvl);
...
const char *trade_letters[] = {
        "Добрый",                 /* First, the alignment based ones */
        "Злой",
        "Нейтральный",
        "Маг",           /* Then the class based ones */
        "Клерик",
        "Вор",
        "Воин",
        "\n"
...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_MAGIC_USER(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
	  ...
	  (IS_TAMPLIER(ch)   && NOTRADE_TAMPLIER(shop_nr))	 || //IS_PALADINE
	  (IS_SLEDOPYT(ch)   && NOTRADE_SLEDOPYT(shop_nr))	 ||
	  (IS_MONAH(ch)		 && NOTRADE_MONAH(shop_nr))	     ||
	  (IS_VARVAR(ch)     && NOTRADE_VARVAR(shop_nr))     ||
	  (IS_DRUID(ch)      && NOTRADE_DRUID(shop_nr))		||
        (IS_WEDMAK(ch)     && NOTRADE_WEDMAK(shop_nr)))
  {
...
int evaluate_expression(struct obj_data * obj, char *expr)
{
  ...
  while (*ptr) {
	...
	while (*ptr && !is_space(*ptr) && (find_oper_num(*ptr) == NOTHING))
	  ptr++;
    ...
    for (counter = 0;
      ...
      if (value_for_mode(shop_nr, counter, mode) == GET_OBJ_TYPE(item))
         {// Used wand or stave
	      ...
	      // Special bits разобраться (все и так ясно, для дальнейшего баланса)
...
char *times_message(struct obj_data * obj, char *name, int num)
{
  ...
  if (obj)
    strcpy(buf, obj->short_vdescription);
  else {
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  if ((buynum = transaction_amt(arg)) < 0) {
    sprintf(buf, "%s! Вы мне сначала продайте эту вещь, а после покупайте!",
	    GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  if (!(*arg) || !(buynum)) {
    sprintf(buf, "%s: Что Вы хотите купить?", GET_NAME(ch)); /*What do you want to buy*/
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  if ((buy_price(obj, ch, shop_nr) > (!SHOP_FOR_GLORY(shop_nr) ? GET_GOLD(ch) : GET_GLORY(ch))) && !IS_GLGOD(ch)) {
  ...
  //    do_action(keeper, GET_NAME(ch), cmd_puke, 0);
      return;
    case 1:
      do_echo(keeper, "вздохнул$y с разочарованием.", cmd_emote, SCMD_EMOTE);
      return;
    default:
      return;
    }
  ...
  if ((IS_CARRYING_N(ch) + 1 > CAN_CARRY_N(ch))) {
    sprintf(buf, "%s, Вам некуда положить %s.\r\n", GET_NAME(ch), obj->short_vdescription);
	    //fname(obj->name)); 
    send_to_char(buf, ch);
    return;
  }
  if ((IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj)) > CAN_CARRY_W(ch)) {
    sprintf(buf, "%s, У Вас не хватает сил еще взять и %s.\r\n",
		GET_NAME(ch), obj->short_vdescription);
	   // fname(obj->name));
    send_to_char(buf, ch);
    return;
  }
        while ((obj) && (((!SHOP_FOR_GLORY(shop_nr) ? GET_GOLD(ch) : GET_GLORY(ch)) >= buy_price(obj, ch, shop_nr)) || IS_GLGOD(ch))
	 && (IS_CARRYING_N(ch) < CAN_CARRY_N(ch)) && (bought < buynum)
	 && (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch))) {
    bought++;

        if (invalid_anti_class(ch, obj))
	  {  sprintf(buf, "Ваша наклонность не позволяет купить %s.\r\n", obj->short_vdescription);
		  send_to_char(buf, ch);
		  continue;
	  }
	 ...
	 if (GET_OBJ_MAX(obj) > decay)
	    ...
	    GET_OBJ_MAX(obj) = 1;
         ...
         goldamt += buy_price(obj, ch, shop_nr);
                ...
                if (!IS_GLGOD(ch)) 
		 {  if (!SHOP_FOR_GLORY(shop_nr))
			  GET_GOLD(ch) -= buy_price(obj, ch, shop_nr); 
                     else 
			  GET_GLORY(ch)-= buy_price(obj, ch, shop_nr);
  ...
  if (bought < buynum) {
    if (!obj || !same_obj(last_obj, obj))
      sprintf(buf, "%s, У меня на продажу их осталось только %d.", GET_NAME(ch), bought); /*I only have %d to sell you.*/
    else if (GET_GOLD(ch) < buy_price(obj, ch, shop_nr))
      sprintf(buf, "%s, Вы можете позволить себе оплатить только %d.", GET_NAME(ch), bought); /*You can only afford */
    else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
      sprintf(buf, "%s, Вы можете удержать только %d.", GET_NAME(ch), bought);/*You can only hold*/
    else if (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch))
      sprintf(buf, "%s, Вы можете унести только %d.", GET_NAME(ch), bought); /*You can only carry*/
    else
      sprintf(buf, "%s, Я Вам могу продать только %d.", GET_NAME(ch),
	      bought);
    do_tell(keeper, buf, cmd_tell, 0);
  }
  if (!IS_GOD(ch))
  ...
  strcpy(tempstr, times_message(obj, 0, bought));
  sprintf(buf, "$n купил$y %s.", tempstr);
  act(buf, FALSE, ch, obj, 0, TO_ROOM);

write_buf(buf, shop_index[shop_nr].message_buy, GET_NAME(ch), goldamt,!SHOP_FOR_GLORY(shop_nr) ? WHAT_MONEYu : WHAT_GLORY); 
  do_tell(keeper, buf, cmd_tell, 0);
  sprintf(buf, "Вы купили %s.\r\n", tempstr); 
  send_to_char(buf, ch);
...
{ char buf[MAX_STRING_LENGTH];
  ...
  {
  case OBJECT_NOVAL:
    sprintf(buf, "%s! %s Вы можете придержать для себя!",
	        CAP(GET_NAME(ch)),obj ? obj->short_vdescription : "Этот хлам!");
    break;
  case OBJECT_NOTOK:
    if (mode == MODE_REPAIR)
       sprintf(buf, "%s! Я не буду чинить %s.", GET_NAME(ch), obj ? obj->short_vdescription : "эту вещь");
    else
    if (mode == MODE_CHANGE)
       sprintf(buf, "%s! Мне не выгодно менять такие вещи.", GET_NAME(ch));
    else
  ...
  case OBJECT_NOTEMPTY:
    sprintf(buf, "%s! В %s что-то лежит", GET_NAME(ch), obj->short_pdescription);
    break;
  default:
		            ...
		            struct char_data * keeper, int shop_nr, int msg)
{
  ...
  switch (result) {
  case OBJECT_NOVAL:
    sprintf(buf, "%s, Вы наверное издеваетесь? Эта вещь гроша ломанного не стоит!!!",
	    GET_NAME(ch)); /*You've got to be kidding, that thing is worthless!*/
    break;
  case OBJECT_NOTOK:
    sprintf(buf, shop_index[shop_nr].do_not_buy, GET_NAME(ch));
    break;
 		                ...
 		                int shop_nr)
{
  ...
  if (!obj)
     {sprintf(buf, "%s! Что Вы хотите мне продать?", GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
  ...
  GET_GOLD(ch) += goldamt;
  sprintf(buf, "$n продал$y %s.", times_message(obj, 0, sold));
  act(buf, FALSE, ch, obj, 0, TO_ROOM);

  write_buf(buf, shop_index[shop_nr].message_sell, GET_NAME(ch), goldamt,WHAT_MONEYa);
  do_tell(keeper, buf, cmd_tell, 0);

  REMOVE_BIT(GET_OBJ_EXTRA(obj, ITEM_LIFTED), ITEM_LIFTED);
  sprintf(buf, "%s купил$Y у Вас %s.",keeper->player.short_descr, times_message(obj, 0, sold));
  act(buf, FALSE, ch, 0, keeper, TO_CHAR);
 // sprintf(buf, "Теперь у %s есть %s.\r\n", keeper->player.rname, times_message(obj, 0, sold));
 // send_to_char(buf, ch);
                     ...
                     int shop_nr, int operation)
{
 ...
 if (!*argument)
    {sprintf(buf, "%s! что Вы хотели бы поменять?", GET_NAME(ch));
     do_tell(keeper, buf, cmd_tell, 0);
     return;
    }
 ...
 while (argument && *argument)
  ...
  if (!srcs)
     {sprintf(buf, "%s! Меня это не устраивает!", GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }

  if ((buynum = transaction_amt(argument)) < 0)
     {sprintf(buf, "%s! Такого предмета у меня нет!\r\n",
	          GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
  ...
  if (!*argument || !buynum)
     {sprintf(buf, "%s! на что Вы хотите поменять?", GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      restore_objects(ch,source_list,srcs);
      return;
     }
      ...
      {if (get_selling_item(ch, source_list[i], keeper, shop_nr, TRUE, MODE_CHANGE))
          {if (GET_OBJ_MAX(source_list[i]) > GET_OBJ_CUR(source_list[i]) * 2)
              {sprintf(buf, "%s! Я не возьму %s в таком ужасном состоянии.",
                       GET_NAME(ch), source_list[i]->short_vdescription);
               do_tell(keeper, buf, cmd_tell, 0);
  ...
  if (!sellprice)
     {sprintf(buf, "%s! Я не могу так меняться! У меня этого предмета нет!",
              GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      restore_objects(ch,source_list,srcs);
      return;
  ...
  if (!count)
     sprintf(buf, "%s! Такой обмен для меня не выгоден!.", GET_NAME(ch));
  else
  if (buynum == 1 && operation == OPER_CHANGE_VALUE)
     sprintf(buf, "%s! Я Вам дам за это %d %s (%s).", GET_NAME(ch),
             count, desc_count(count, WHAT_THINGa), obj->short_description);
  else
     sprintf(buf, "%s! Хорошо! Этот обмен для меня будет подходящим.", GET_NAME(ch));
  ...
  while (obj && bought < buynum && dests < MAX_LIST_LENGTH)
        {bought++;
         ...
         if (shop_producing(obj, shop_nr))
            {//obj = read_object(GET_OBJ_RNUM(obj), REAL);
             obj = read_object(GET_OBJ_VNUM(obj), VIRTUAL);
             load_otrigger(obj);
             dest_list[dests++] = obj;
            }
         else
  ...
  if (IS_CARRYING_N(ch) + items_inc - items_dec >= CAN_CARRY_N(ch))
     sprintf(buf,"%s! Вы не можете удержать столько предметов", GET_NAME(ch));
  else
  if (IS_CARRYING_W(ch) + weight_inc - weight_dec >= CAN_CARRY_W(ch))
     sprintf(buf,"%s! У Вас нет сил для этого", GET_NAME(ch));
  else
     *buf = '\0';
  ...
  if (*buf)
     ...
     {restore_objects(ch,dest_list,dests);
      ...
      sprintf(buf,"%s! Идет!!!",GET_NAME(ch));
     }
  do_tell(keeper,buf,cmd_tell,0);
}
...
{ struct obj_data *obj;
  ...
  if (transaction_amt(arg) < 0)
     {sprintf(buf, "%s! Но у меня нет такого предмета.\r\n",
	          GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
    if (!*arg)
     if (!operation) 
	 { sprintf(buf, "%s! Что Вы хотите посмотреть?", GET_NAME(ch));
       do_tell(keeper, buf, cmd_tell, 0);
       return;
     }
	 else
	 { sprintf(buf, "%s! Свойства какого товара Вы хотели узнать?", GET_NAME(ch));
       do_tell(keeper, buf, cmd_tell, 0);
       return;
	 }
  ...
  if (!(obj = get_purchase_obj(ch, arg, keeper, shop_nr, TRUE)))
     return;
            if (operation)
	       {   if (GET_BANK_GOLD(ch) >= 50)
			{  GET_BANK_GOLD(ch) -= 50;
			   send_to_char("С Вашего банковского счета было снято 50 монет за эту услугу!\r\n", ch);	
			}
		   else 
			   if (GET_GOLD(ch) >= 50)
			   { GET_GOLD(ch) -= 50; 
				send_to_char("За эту услугу, Вы заплатили 50 монет.\r\n", ch);	
			   }
		   else 
		   { send_to_char("Эта услуга стоит 50 монет и у Вас их нет!\r\n", ch);
				return;		 
		   }		 
		   if (obj)
  ...
  if (*buf2)
     {sprintf(buf,"%s! %s",GET_NAME(ch),buf2);
      act(diag_timer_to_char(obj),FALSE,ch,obj,0,TO_CHAR);
      do_tell(keeper, buf, cmd_tell, 0);
     };
}
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  if (!obj)
     {sprintf(buf, "%s! Что Вы хотите оценить?", GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
  one_argument(arg, name);
  ...
  if (GET_OBJ_MAX(obj) > GET_OBJ_CUR(obj) * 2)
     {sprintf(buf, "%s! Я не буду покупать %s в таком ужасном состоянии.",
              GET_NAME(ch), obj->short_vdescription);
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
  ...
  sprintf(buf, "%s! Я дам Вам за %s %d %s!",
          GET_NAME(ch), VOBJS(obj,ch),
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  if (!(*arg)) {
    sprintf(buf, "%s, что Вы хотите оценить?", GET_NAME(ch)); /*What do you want me to evaluate*/
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  one_argument(arg, name);
  ...
  sprintf(buf, "%s, Я дам Вам %d монет за %s!", GET_NAME(ch),
	  sell_price(obj, ch, shop_nr), obj->short_vdescription); /*I'll give you %d gold coins for that*/
  do_tell(keeper, buf, cmd_tell, 0);
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  if (!obj)
     {sprintf(buf, "%s! Что Вы хотите починить?", GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
  one_argument(arg, name);
...
/*      OBJ_FLAGGED(obj, ITEM_NORENT) ||*/
      ...
      {sprintf(buf,"%s! Мне некогда тратить свое время на %s.",
               GET_NAME(ch), VOBJS(obj,ch));
       do_tell(keeper, buf, cmd_tell, 0);
       return;
      }

  sprintf(buf, "%s! Починка %s Вам обойдется в %d %s",
                GET_NAME(ch), ROBJS(obj,ch), price, desc_count(price, WHAT_MONEYu));
  do_tell(keeper, buf, cmd_tell, 0);

  if (!IS_GOD(ch) && price > GET_GOLD(ch))
     {act("Вы не в состоянии мне заплатить за мою работу!.",FALSE,ch,0,0,TO_CHAR);
      return;
     }
  ...
  if (!IS_GOD(ch))
     GET_GOLD(ch)  -= price;
  act("$n мастерски починил$y $3.",FALSE,keeper,obj,0,TO_ROOM);
  GET_OBJ_CUR(obj)  = GET_OBJ_MAX(obj);
                ...
                struct char_data *keeper, int shop_nr)
{
  ...
  {   multi = atoi(arg);
      ...
      if (multi <= 0)
      { send_to_char("Это не имеет смысла.\r\n", ch);
        return;
      }
     ...
     { 
      ...
      {  
          bool repeair = false;
          if (subcmd == SCMD_REPAIR)
          {
              for (int i = 0; i < NUM_WEARS; i++)
              {
                  if (GET_EQ(ch, i))
                  { repeair = true;
                    shopping_repair_item(GET_EQ(ch, i),ch,keeper,shop_nr);
                  }
              }                            
          }
      ...
      if (dotmode == FIND_ALLDOT)
         {if (!*arg)
          ...
          if (!(obj = get_obj_in_list_vis(ch, arg, ch->carrying)))
             {sprintf(buf, "У Вас нет ничего похожего на этот предмет.\r\n");
	          send_to_char(buf, ch);
             }
          while (obj)
             {next_obj = get_obj_in_list_vis(ch, arg, obj->next_content);
...
char *list_object(struct obj_data * obj, int cnt, int index, int shop_nr)
{
  ...
  if (shop_producing(obj, shop_nr))
    strcpy(buf2, "Неограничено   ");
  else
    sprintf(buf2, "%5d       ", cnt);
  sprintf(buf, " %2d)  %15s", index, buf2);
  ...
  if ((GET_OBJ_TYPE(obj) == ITEM_WAND) || (GET_OBJ_TYPE(obj) == ITEM_STAFF))
    if (GET_OBJ_VAL(obj, 2) < GET_OBJ_VAL(obj, 1))
      strcat(buf3, " (частитчно использовано)");
...
/*void shopping_list(char *arg, struct char_data * ch,
		        struct char_data * keeper, int shop_nr)
{
  char buf[MAX_STRING_LENGTH+MAX_STRING_LENGTH], name[MAX_INPUT_LENGTH];
  struct obj_data *obj, *last_obj = 0;
  int cnt = 0, index = 0;

  if (shop_nr < 0)
  { sprintf(buf, "Ошибка магазина: Хозяин %s, покупатель %s %d",GET_NAME(keeper), GET_NAME(ch), GET_ROOM_VNUM(IN_ROOM(ch)));
  ...
  one_argument(arg, name);
  strcpy(buf, "  ##    Количество   Наименование                                       Цена\r\n");
  strcat(buf, "-----------------------------------------------------------------------------\r\n");
  if (keeper->carrying)
  ...
  if (!last_obj) {
    if (*name)
      strcpy(buf, "Этого нет в продаже!\r\n");
    else
      strcpy(buf, "В настоящее время, в продаже ничего нет.\r\n");
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  one_argument(arg, name);
  buf = NULL;
  buf = str_add(buf, "  ##    Количество   Наименование                                       Цена\r\n");
  buf = str_add(buf, "-----------------------------------------------------------------------------\r\n");
  if (keeper->carrying)
    for (obj = keeper->carrying; obj; obj = obj->next_content)
      if (CAN_SEE_OBJ(ch, obj) && (obj->obj_flags.cost > 0))
  ...
  if (!last_obj)
    ...
    if (*name)
      buf = str_add(buf, "Этого нет в продаже!\r\n");
    else
      buf = str_add(buf, "В настоящее время, в продаже ничего нет.\r\n");
	}
   else 
	  if (!(*name) || isname(name, last_obj->name))
      buf = str_add(buf, list_object(last_obj, cnt, index, shop_nr));
...
SPECIAL(shop_keeper)
{
  ...
  if (CMD_IS("украсть")||CMD_IS("steal")) 
   { sprintf(argm, "$N закричал$Y: \"%s\"", MSG_NO_STEAL_HERE); 
  ...
  if (CMD_IS("repair") || CMD_IS("ремонт"))
...
void read_line1(FILE * shop_f, const char *string, bool tr)
{ 
  char data[256];
  ...
  if (tr)
    asciiflag_conv1(data, &SHOP_BITVECTOR(top_shop));
  else
    asciiflag_conv1(data, &SHOP_TRADE_WITH(top_shop));
...
void list_all_shops(struct char_data * ch)
{
  ...
  for (shop_nr = 0; shop_nr <= top_shop; shop_nr++) {
    if (!(shop_nr % 19)) {
      strcat(buf, " ## Виртуальный  Где     Хозяин  Купить Продать Обмен  Торгует с\r\n");
      strcat(buf, "-----------------------------------------------------------------\r\n");
    }
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  strcpy(buf, "Хозяин магазина: "); /*Shopkeeper*/
  if (SHOP_KEEPER(shop_nr) >= 0) {
    ...
    if ((k = get_char_num(SHOP_KEEPER(shop_nr)))) {
      send_to_char(buf, ch);
      sprintf(buf, "Монеты:  [%9d]. Банк: [%9d]. Всего: [%d].\r\n",
	 GET_GOLD(k), SHOP_BANK(shop_nr), GET_GOLD(k) + SHOP_BANK(shop_nr)); /*Coins*/
    }
  } else
    strcat(buf, "<НЕТ>\r\n"); 
  send_to_char(buf, ch);
...
void show_shops(struct char_data * ch, char *arg)
{
  ...
  if (!*arg)
    ...
    if (shop_nr < 0 || shop_nr > top_shop) {
      send_to_char("Неправельный номер магазина.\r\n", ch);
      return;
    }
