...
int do_npc_rescue(struct char_data * ch_hero, struct char_data * ch_victim)
{
  ...
  act("Вы героически спасаете $V.\r\n", FALSE, ch_hero, 0, ch_victim, TO_CHAR);
  act("Вы были спасены $V, Вам стало неловко!\r\n",/*You are rescued by $N, your loyal friend*/
	FALSE, ch_victim, 0, ch_hero, TO_CHAR);
  act("$n героически спас$q $V.", FALSE, ch_hero, 0, ch_victim, TO_NOTVICT);

  if (FIGHTING(ch_victim)) {
	stop_fighting(ch_victim, FALSE);
	WAIT_STATE(ch_victim, PULSE_VIOLENCE);
    }
...
SPECIAL(king_welmar)
{
  ...
  case '4':
  case '5':
    perform_move(ch, path[index] - '0',0, FALSE);
    break;
  ...
  case 'W':
    ...
    act("$n проснул$u и встает.", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'S':
    GET_POS(ch) = POS_SLEEPING;
    act("$n ложится на $m красивую кровать и тут же засыпает.", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'r':
    GET_POS(ch) = POS_SITTING;
    act("$n садится на $m большой трон.", FALSE, ch, 0, 0, TO_ROOM);
    break;
  ...
  case 'G':
    act("$n говорит \"Доброе утро, дорогие друзья.\"", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'g':
    act("$n говорит \"Доброе утро, дорогие жители.\"", FALSE, ch, 0, 0, TO_ROOM);
    break;
...
SPECIAL(training_master)
{
  ...
  case 6:
    act("$n попытал$u пнуть $V, но промахнул$u.", FALSE, pupil1, 0, pupil2, TO_NOTVICT);
    act("Вы попытались пнуть $V, но промахнулись.", FALSE, pupil1, 0, pupil2, TO_CHAR);
    act("$n попытал$u пнуть Вас, но промахнул$u.", FALSE, pupil1, 0, pupil2, TO_VICT);
    break;
...
int castle_cleaner(struct char_data *ch, int cmd, int gripe)
{
  ...
  for (i = world[ch->in_room].contents; i; i = i->next_content) {
    ...
    if (gripe) {
      act("$n сказал$y: 'My oh my!  I ought to fire that lazy cleaning woman!'",
          FALSE, ch, 0, 0, TO_ROOM);
      act("$n подобрал$y кучу мусора.", FALSE, ch, 0, 0, TO_ROOM);
    }
    obj_from_room(i);
    obj_to_char(i, ch);
    return (TRUE);
...
SPECIAL(peter)
{
  ...
  if (!(number(0, 3)) && (ch_guard = find_guard(ch)))
    ...
    case 0:
      ...
      act("Вы идете очень внимательно, так как $n следит за Вами.",
    ...
    case 2:
      ...
      break;
    case 3:
      act("$n смотрел$y на Вас.", FALSE, ch, 0, ch_guard, TO_VICT);
      act("$n смотрел$y на $N.", FALSE, ch, 0, ch_guard, TO_NOTVICT);
      act("$n заорал$y: 'Those boots need polishing!'",
    ...
    case 4:
      act("$n смотрел$y на Вас.", FALSE, ch, 0, ch_guard, TO_VICT);
      act("$n смотрел$y на $N.", FALSE, ch, 0, ch_guard, TO_NOTVICT);
      act("$n заорал$y: 'Straighten that collar!'",
	  FALSE, ch, 0, ch_guard, TO_ROOM);
      act("You growl at $N.", FALSE, ch, 0, ch_guard, TO_CHAR);
      break;
    default:
      act("$n смотрел$y на Вас.", FALSE, ch, 0, ch_guard, TO_VICT);
      act("$n смотрел$y на $N.", FALSE, ch, 0, ch_guard, TO_NOTVICT);
      act("$n заорал$y: 'That chain mail looks rusty!  CLEAN IT !!!'",
	  FALSE, ch, 0, ch_guard, TO_ROOM);
      act("You growl at $N.", FALSE, ch, 0, ch_guard, TO_CHAR);
      break;
    }
