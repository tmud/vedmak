...
extern void asciiflag_conv1(char *flag, void *value);
...
void parse_trigger(FILE *trig_f, int nr)
{
   ...
   // asciiflag_conv1(flags, &trig->trigger_type);
    trig->narg = (k == 3) ? t[0] : 0;
...
/* release memory allocated for a script вышефункцию убрать перенести в линух*/
...
{int spell = GET_SPELL_TYPE(ch, spellnum);
 ...
 if (spell & SPELL_KNOW)
    {if (spelldiff)
        ...
        {sprintf(buf,"Вы полностью забыли заклинание \"%s\".\r\n", spell_name(spellnum));
         send_to_char(buf,ch);
    ...
    {if (spelldiff)
        {sprintf(buf,"Вы познали заклинание \"%s\".\r\n", spell_name(spellnum));
         send_to_char(buf,ch);
